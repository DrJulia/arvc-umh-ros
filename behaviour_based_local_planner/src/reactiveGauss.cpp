...
reactiveGauss::reactiveGauss(const ConfigFile& config):
	...
	eszdilationradius		((int)floor(config.read<float>("ESZDILATIONRADIUS")/config.read<float>("RESOLUTION")+0.5)),
	actionradius			((int)floor(config.read<float>("ACTIONRADIUS")/config.read<float>("RESOLUTION")+0.5)),
	endReactive			(true),
...
void reactiveGauss::updateField (localPotentialField& global){
	...
	binMap esz;
	if (useesz){
		omap->esz(poscell.x, poscell.y, esz, eszdilationradius,actionradius);
	}
	else{
		esz.initialize(omap->getWidth(), omap->getHeight(), omap->getResolution(), omap->getXOrigin(), omap->getYOrigin());
		for(int i=poscell.x-actionradius; i < poscell.x+actionradius; i++){
			for(int j=poscell.y-actionradius; j < poscell.y+actionradius; j++){
				esz.set(i,j,true);
			}
		}
		RoI roi;
		roi.x      = (poscell.x-actionradius>0)?                  poscell.x-actionradius                      : roi.x = 0;
		roi.width  = (poscell.x+actionradius<omap->getWidth())?   poscell.x+actionradius - roi.x              : omap->getWidth() - roi.x;
		roi.y      = (poscell.y-actionradius>0)?                  roi.y = poscell.y-actionradius              : roi.y = 0;
		roi.height = (poscell.y+actionradius< omap->getHeight())? roi.height = poscell.y+actionradius - roi.y : roi.height = omap->getHeight() - roi.y;
		esz.setRoi(roi);
	}
	...
	localPotentialField* goImp = new localPotentialField(lzwidth,lzheight); 
	processGoImp(poscell, *ipmap, esz, *goImp);
	global += *goImp * goToImprecisePosesWeight;
	delete goImp;
			...
			for (j = esz.getRoi().y-1; j <= esz.getRoi().y + esz.getRoi().height ; j++){			
				...
				if(omap.isunknown(i,j) && goToUnexploredZonesEnabled) {					// unexplored
					if (esz.get(i,j)){ // if esz
						...
						for (x = 0, ci = pos.x - wws ; ci <= pos.x + wws ; ci++, x++){
							for (y = 0, cj = pos.y - whs ; cj <= pos.y + whs ; cj++, y++){   // for each local cell				
								...
								goUZ.set(x,y, goUZ.get(x,y)-gauss(ci,cj, (float)i,(float)j, goToUnexploredZonesWidth));
							}
						}
					}
				}
					...
					if (esz.get(i,j)){ // if esz
						for (x = 0, ci = pos.x - wws ; ci <= pos.x + wws ; ci++, x++){
							for (y = 0, cj = pos.y - whs ; cj <= pos.y + whs ; cj++, y++){   // for each local cell				
								goFro.set(x,y, goFro.get(x,y)-gauss(ci,cj,(float) i,(float)j, goToFrontierWidth));
							}
						}
					}
				}
					...
					if (esz.isOver(i,j,eszdilationradius)){ // if next to esz
						for (x = 0, ci = pos.x - wws ; ci <= pos.x + wws ; ci++, x++){
							for (y = 0, cj = pos.y - whs ; cj <= pos.y + whs ; cj++, y++){   // for each local cell				
								avObs.set(x,y, avObs.get(x,y)+ gauss(ci,cj,(float) i,(float)j, avoidObstaclesWidth));
							}
						}
					}
				}
		...
		for (i = esz.getRoi().x ; i < esz.getRoi().x + esz.getRoi().width-1 ; i++){
			for (j = esz.getRoi().y ; j < esz.getRoi().y + esz.getRoi().height-1 ; j++){			
				...
				if(ppmap.get(i,j) && esz.get(i,j)) {	
					for (x = 0, ci = pos.x - wws ; ci <= pos.x + wws ; ci++, x++){
						for (y = 0, cj = pos.y - whs ; cj <= pos.y + whs ; cj++, y++){   // for each local cell				
							goPre.set(x,y, goPre.get(x,y)-gauss(ci,cj, (float)i,(float)j, goToPrecisePosesWidth));
						}
					}
				}
			}
...
void reactiveGauss::processGoImp(point& pos, binMap& ppmap, binMap&  esz, localPotentialField& goPre){

	if(goToImprecisePosesEnabled){
		int i,j,ci,cj,x,y;
		int wws = (lzwidth-1)/2;
		int whs = (lzheight-1)/2;	
		...
		for (i = esz.getRoi().x ; i < esz.getRoi().x + esz.getRoi().width-1 ; i++){
			for (j = esz.getRoi().y ; j < esz.getRoi().y + esz.getRoi().height-1 ; j++){			
				...
				if(ppmap.get(i,j) && esz.get(i,j)) {	
					for (x = 0, ci = pos.x - wws ; ci <= pos.x + wws ; ci++, x++){
						for (y = 0, cj = pos.y - whs ; cj <= pos.y + whs ; cj++, y++){   // for each local cell				
							goPre.set(x,y, goPre.get(x,y)-gauss(ci,cj, (float)i,(float)j, goToImprecisePosesWidth));
						}
					}
				}
			}
...
void reactiveGauss::processGoGoal(point& pos, point& goal, binMap& esz, localPotentialField& goGoal){
	...
	//	if(esz.get(goal.x,goal.y)) {	
			for (x = 0, ci = pos.x - wws ; ci <= pos.x + wws ; ci++, x++){
				for (y = 0, cj = pos.y - whs ; cj <= pos.y + whs ; cj++, y++){   // for each local cell				
					goGoal.set(x,y, goGoal.get(x,y)-gauss(ci,cj, goal.x,goal.y, goToGoalWidth));
				}
			}
	//	}
	}
					...
					if (esz.get(robotcells[r2].x, robotcells[r2].y)){	
						for (x = 0, ci = robotcells[r].x - wws ; ci <= robotcells[r].x + wws ; ci++, x++){
							for ( y = 0, cj = robotcells[r].y - whs ; cj <= robotcells[r].y + whs ; cj++, y++){   // for each local cell				
								avRob.set(x,y, avRob.get(x,y)+gauss(ci,cj, robotcells[r2].x, robotcells[r2].y, avoidOtherRobotsWidth));
							}
						} // local cell j
					}
				}// r2 != r
