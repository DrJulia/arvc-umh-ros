...
pose4 base2global(const pose4& base, const pose& pos){
	float costh = (float)cos(pos.th);
	float sinth = (float)sin(pos.th);
	return pose4(	costh * base.x - sinth * base.x + pos.x,
...
// TODO: test & debug
matrix gaussianSample3d(const matrix& mean, const matrix& sigma){
	// Find any real matrix A such that A AT = Σ. When Σ is positive-definite, the Cholesky decomposition is typically used. 
	// In the more general nonnegative-definite case, one can use the matrix A = UΛ½ obtained from a spectral decomposition Σ = UΛUT of Σ.
	CvMat* s = cvCreateMat(sigma.getRows(),sigma.getCols(),CV_32FC1);
	CvMat* w = cvCreateMat(sigma.getRows(),sigma.getCols(),CV_32FC1);
	CvMat* u = cvCreateMat(sigma.getRows(),sigma.getCols(),CV_32FC1);
	CvMat* v = cvCreateMat(sigma.getRows(),sigma.getCols(),CV_32FC1);
	for (int r = 0; r<sigma.getRows(); r++)
		for (int c = 0; c<sigma.getCols(); c++)
			cvmSet(s,r,c, sigma.get(r,c));
	cvSVD(s, w, u, v);
	matrix W(sigma.getRows(),sigma.getCols());
	for (int r = 0; r<sigma.getRows(); r++)
		for (int c = 0; c<sigma.getCols(); c++)
			W.set(r,c, (float)cvmGet(w,r,c));
	matrix U(sigma.getRows(),sigma.getCols());
	for (int r = 0; r<sigma.getRows(); r++)
		for (int c = 0; c<sigma.getCols(); c++)
			U.set(r,c, sqrt((float)cvmGet(u,r,c)));
	matrix V(sigma.getRows(),sigma.getCols());
	for (int r = 0; r<sigma.getRows(); r++)
		for (int c = 0; c<sigma.getCols(); c++)
			V.set(r,c, (float)cvmGet(v,r,c));
	matrix A = U*W;
...
matrix::matrix(const geometry_msgs::Pose &p): rows(4), cols(1), val(new float[4]){
	...
	set(2,0,tf::getYaw(p.orientation) );
}
