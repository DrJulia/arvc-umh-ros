...
//		ROS_INFO("new line\n");
		for (int k = 1; k < size; k++){
			ii = line[k].x; jj = line[k].y;
			if (ii>=0 && jj>=0 && ii<width && jj< height){
				if(visible == true && (!isfree(ii,jj,dilaterad) || (x-ii)*(x-ii)+(y-jj)*(y-jj)>max2)){ visible = false; break;}
//				ROS_INFO("[%d,%d] visible = %d\n",ii,jj,visible);
				map.set(ii,jj,visible);
				if (visible){
					if(ii > roi.x+roi.width-1)		roi.width = ii-roi.x+1;
...
//		printf("[ESZ] get line OK\n");
		for (int k = 1; k < size; k++){
			ii = line[k].x; jj = line[k].y;
			if (ii>=0 && jj>=0 && ii<width && jj< height){
				if(visible == true && (!isfree(ii,jj,dilaterad) || (x-ii)*(x-ii)+(y-jj)*(y-jj)>max2)) visible = false;
				map.set(ii,jj,visible);
				if (visible){
					if(ii > roi.x+roi.width-1)		roi.width = ii-roi.x+1;
					else if(ii < roi.x) 			{roi.width  += roi.x-ii; roi.x    = ii;}
