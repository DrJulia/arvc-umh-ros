...
//	void initialize(float w, float h, float res, float x, float y);
	...
	/// get pixel value
	bool get(const uint& x, const uint& y) const;
	/// set pixel value
	void set(const int& x, const int& y, bool value);
	/// set pixel value
	void set(const uint& x, const uint& y, bool value);
	/// set cells from cluster cell
	void set(const std::vector<clusterCell>& cl);

	/// checks if all cells in a mask of defined size is true 
	bool isInside(const int& x, const int& y, const int& size) const; 
	/// checks if a 1px diamant over that cell is inside the binmap
	bool isInsideD(const int& x, const int& y) const; 
	/// Checks if at least one cell in a mask of defined size is true
	bool isOver(const int& x, const int& y, const int& size) const;
	/// Checks if at least one cell in a mask of defined size is true
	bool isOver(const uint& x, const uint& y, const uint& size) const;
	bool isOverNotSafe(const uint& x, const uint& y, const uint& size) const;
...
private:
	/// auxiliary metho to remove unconnected pixels
	void remUnconnectRec(int x, int y, const binMap& prevData);

	bool clustering(const uint &i, const uint &j, binMap& aux, clusterCell& cl, std::list<point> &seq, bool backfront) const;
...
inline RoI& binMap::getRoi() 				{return roi;}
inline void binMap::setRoi(const RoI& r)		{this->roi = r;}

inline bool binMap::get(const int& x, const int& y) const{
	int idx = y*width+x;
	div_t idx32 = div (idx, 32);
	return (idx32.quot>=0 && idx32.quot<(int)totalsize) ? cells[idx32.quot][idx32.rem] : false;
}
inline bool binMap::get(const uint& x, const uint& y) const{
	int idx = y*width+x;
	div_t idx32 = div (idx, 32);
	return (idx32.quot<(int)totalsize) ? cells[idx32.quot][idx32.rem] : false;
}

inline void binMap::set(const int& x, const int& y, bool value){
	int idx = y*width+x;
	div_t idx32 = div (idx, 32);
	if (idx32.quot>=0 && idx32.quot<(int)totalsize) cells[idx32.quot].set(idx32.rem,value);
}

inline void binMap::set(const uint& x, const uint& y, bool value){
	int idx = y*width+x;
	div_t idx32 = div (idx, 32);
	if (idx32.quot<(int)totalsize) cells[idx32.quot].set(idx32.rem,value);
}
