...
typedef struct pose{
	...
	pose(const tf::Pose& p): x(p.getOrigin().getX()),y(p.getOrigin().getY()),th(tf::getYaw(p.getRotation())){}
	pose(const geometry_msgs::Pose& p): x(p.position.x),y(p.position.y),th(tf::getYaw(p.orientation) ){}
	pose& operator= (const pose& p) {x = p.x; y = p.y; th = p.th; return *this;}
	pose& operator= (const tf::Pose& p) {x =p.getOrigin().getX(); y = p.getOrigin().getY(); th = tf::getYaw(p.getRotation()); return *this;}
	pose  operator+ (const pose& p) const {return pose(x+p.x, y+p.y, th+p.th);}
	pose  operator- (const pose& p) const {return pose(x-p.x, y-p.y, th-p.th);}
	pose& operator+=(const pose& p) {x+=p.x; y+=p.y; th+=p.th; return *this;}
	pose& operator-=(const pose& p) {x-=p.x; y-=p.y; th-=p.th; return *this;}
...
/// pose4
typedef struct pose4{
	float x; 
	float y; 
	float z; 
	float th;
	pose4():x(0.0f),y(0.0f),z(0.0f),th(0.0f){}
	pose4(float nx, float ny, float nz, float nth): x(nx),y(ny),z(nz),th(nth){}
	pose4(const pose4& p): x(p.x),y(p.y),z(p.z),th(p.th){}
	pose4(const tf::Pose& p): x(p.getOrigin().getX()),y(p.getOrigin().getY()),z(p.getOrigin().getZ()),th(tf::getYaw(p.getRotation())){}
	pose4(const geometry_msgs::Pose& p): x(p.position.x),y(p.position.y),z(p.position.z),th(tf::getYaw(p.orientation) ){}	
	pose4& operator= (const pose4& p) {x = p.x; y = p.y; z = p.z; th = p.th; return *this;}
	pose4& operator= (const tf::Pose& p) {x =p.getOrigin().getX(); y = p.getOrigin().getY(); z = p.getOrigin().getZ(); th = tf::getYaw(p.getRotation()); return *this;}
	pose4  operator+ (const pose4& p) const {return pose4(x+p.x, y+p.y, z+p.z, th+p.th);}
	pose4  operator- (const pose4& p) const {return pose4(x-p.x, y-p.y, z-p.z, th-p.th);}
	pose4& operator+=(const pose4& p) {x+=p.x; y+=p.y; z+=p.z; th+=p.th; return *this;}
	pose4& operator-=(const pose4& p) {x-=p.x; y-=p.y; z-=p.z; th-=p.th; return *this;}
	void print(char* str=0) const {printf("%sx: %f, y: %f, z: %f, th: %f\n",str,x,y,z,th);}
	pos3d toPos3d(){return pos3d(x,y,z);}
}pose4;

static pose4 nullpose4;
...
typedef struct RoI{
	...
	RoI(uint nx, uint ny, uint nw, uint nh): x(nx),y(ny),width(nw),height(nh){}
	uint x;
	uint y;
	uint width;
	uint height;
