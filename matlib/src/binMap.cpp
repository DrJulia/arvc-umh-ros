...
bool binMap::isInside(const int& x, const int& y, const int& size) const{
...
bool binMap::isInsideD(const int& x, const int& y) const {
	int i,j;
	i = x; j = y;
	if (!get(i,j))
		return false;
...
bool binMap::isOver(const int& x, const int& y, const int& size) const {
	for (int i = x-size; i<= x+size; i++)
		for (int j = y-size; j<= y+size; j++)
...
bool binMap::isOver(const uint& x, const uint& y, const uint& size) const {
	for (uint j = y-size; j<= y+size; j++){
		uint idx = j*width;
		for (uint i = x-size; i<= x+size; i++){
			div_t idx32 = div (idx+i, 32);
			if (idx32.quot<(int)totalsize && cells[idx32.quot][idx32.rem]) return true;
		}
	}
	return false;
}
...
bool binMap::isOverNotSafe(const uint& x, const uint& y, const uint& size) const {
	for (uint j = y-size; j<= y+size; j++){
		uint idx = j*width;
		for (uint i = x-size; i<= x+size; i++){
			div_t idx32 = div (idx+i, 32);
			if (cells[idx32.quot][idx32.rem]) return true;
		}
	}
	return false;
}
...
//			if (map.get(i,j)) set(i,j,false);
//	for (int i = 0; i< (int)cells.size(); i++){
//		cells[i] &= (~map.cells[i]);
//	}

	for (uint i = roi.y; i < roi.y+roi.height ; i++){
		div_t first = div (roi.x+i*width, 32);
		div_t end = div (roi.x+roi.width+i*width, 32);
		for (int idx = first.quot; idx <= end.quot; idx++)
			cells[idx] &= (~map.cells[idx]);
	}
...
//	for (int i = map.roi.x ; i < map.roi.x + map.roi.width ; i++)
//		for (int j = map.roi.y ; j < map.roi.y + map.roi.height ; j++)    for each cell				
//			if (get(i,j)) 
//				set(i,j, map.get(i,j) );
//	for (int i = 0; i< (int)cells.size(); i++){
//		cells[i] &= map.cells[i];
//	}
	...
	for (uint i = map.roi.y; i < map.roi.y+map.roi.height ; i++){
		div_t first = div (map.roi.x+i*width, 32);
		div_t end = div (map.roi.x+map.roi.width+i*width, 32);
		for (int idx = first.quot; idx <= end.quot; idx++)
			cells[idx] &= map.cells[idx];
	}
...
void binMap::dilate(uint rad){
	uint i,j;
	binMap prevData(*this);
	roi.x = (roi.x >= rad)? roi.x - rad: 0 ;
	roi.y = (roi.y >= rad)? roi.y - rad: 0 ;
	roi.width += 2*rad;
	roi.height += 2*rad;
	...
	for (i = roi.x ; i < roi.x + roi.width ; i++)
		for (j = roi.y ; j < roi.y + roi.height ; j++)   // for each cell				
			if(prevData.isOverNotSafe(i,j,rad)) set(i,j,true);
}
void binMap::erode(uint rad){
	uint i,j;
	binMap prevData(*this);
	roi.x += rad;
	roi.y += rad;
	roi.width  = (roi.width > rad)?  roi.width  - 2*rad : 0;
	roi.height = (roi.height > rad)? roi.height - 2*rad : 0;
	for (i = roi.x ; i < roi.x + roi.width ; i++)
		for (j = roi.y ; j < roi.y + roi.height ; j++)   // for each cell				
			if(!prevData.isInside(i,j,rad)) set(i,j, false);
}
...
IplImage* binMap::getMapAsImage() const {
	...
	for (uint i = roi.x; i< roi.x+roi.width; i++)
		for(uint j = roi.y; j< roi.y+roi.height; j++){
...
int binMap::cluster(vector<clusterCell>& clusterList, int minSize) const{
	...
	for (uint i = getRoi().x ; i < getRoi().x + getRoi().width ; i++){
		for (uint j = getRoi().y ; j < getRoi().y + getRoi().height ; j++){   // for each cell
...
bool binMap::clustering(const uint &i, const uint &j, binMap& aux, clusterCell& cl, std::list<point> &seq, bool backfront) const{
